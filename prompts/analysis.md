...
的 `prompts/analysis.md`（MIT，© zarazhangrui），针对 Bilibili 中文 AI 字幕做了改写。
...
```
你是我的内容助理。我在看一个 Bilibili 视频，请阅读下面的字幕，产出一份结构化概览：章节 + 金句。
...
Bilibili 的 AI 字幕有两个特点，会直接影响你的工作：
...
视频标题：{videoTitle}
UP 主：{ownerName}
输出语言：{outputLanguage}（章节标题、摘要和金句都必须使用此语言）。
用户可调整的概览要求（在不违反时间戳与 JSON 格式约束的前提下优先遵循）：
{customInstructions}
{rangeNote}
...
- `{outputLanguage}` — 概览输出语言，由界面语言决定
- `{customInstructions}` — 用户在概览页调整的生成要求
