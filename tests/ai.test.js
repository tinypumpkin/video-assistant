...
test("视频问答短字幕保留全文和时间戳", () => {
  const context = AI.buildChatTranscriptContext(
    [
      { start: 0, text: "开场介绍" },
      { start: 65, text: "核心结论" },
    ],
  ...
  assert.match(context, /\[0:00\] 开场介绍/);
  assert.match(context, /\[1:05\] 核心结论/);
});

test("视频问答长字幕不会只截开头，会保留相关段和结尾", () => {
  const segments = Array.from({ length: 120 }, (_, index) => ({
    start: index * 5,
    text: index === 95 ? "这里解释量子纠缠的核心机制" : `普通内容第 ${index} 段`,
  }));
  const context = AI.buildChatTranscriptContext(segments, "量子纠缠是什么？", {
    maxChars: 2_000,
  });
  assert.match(context, /量子纠缠的核心机制/);
  assert.match(context, /普通内容第 119 段/);
