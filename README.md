...
> ⚠️ **注意**：本项目为个人开发的开源扩展，当前通过 GitHub Releases 分发。所有 AI 与字幕服务使用你自己的 API Key，扩展本身无后端、无账户、无遥测。
...
- 划词解释选中内容
- 概览提示词可调、自动保存、恢复默认
- **多模态模型支持**：自动探测 Provider 视觉能力，视觉请求与纯文本请求按能力分流

### 🪄 AI 视频笔记 AI Video Note
- 一键把整段字幕整理为结构化复习笔记
- **生成时参考手记截图**：自动挑选当前视频的有效截图（像素质量过滤 + 感知哈希去重，至多 6 张）随视觉请求发给多模态模型；正文中的 `{{va-cite}}` 引用标记在展示时替换为可点击的截图卡片（含时间戳与视频跳转链接）
- 纯文本模型自动回退纯字幕生成，不受影响
...
- **手记**：随手记录 + 视频帧**截图**，视频页附标题+播放位置锚点；保存成功右下角弹卡片（时间戳/标题/复制链接）
- 默认按当前视频过滤，「全部」才跨视频
- 批量删除、按范围清空（二次确认）
- 导出 TXT / CSV / Markdown
- **笔记思维导图**：AI 视频笔记一键转为思维导图视图（自研 `note-mindmap.js` + markmap 渲染）
...
- 已注入按钮随语言切换原地刷新
- 两站 Video Assistant 侧边栏按钮统一使用 SVG icon 图标
