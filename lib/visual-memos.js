...
var BILI_VISUAL_MEMOS = (() => {
  ...
  function selectReferences(notes, resource, { maxImages = 6, maxDataChars = 2_000_000 } = {}) {
    const candidates = (Array.isArray(notes) ? notes : [])
      ...
      .map((note) => ({
        ...
        memoText: String(note.text || "").trim().slice(0, 2_000),
  ...
  function timestampSeconds(source) {
    const match = String(source || "").match(/\[(\d{1,3}):([0-5]\d)(?::([0-5]\d))?\]/);
  ...
  /**
   * 模型偶尔会把合法引用集中放到摘要或文末。只移动模型实际使用过的引用，
   * 按截图时间将它确定性地放到时间戳最接近的正文行之后；正文没有任何
   * 时间戳时保留模型原位置，避免在没有证据的情况下猜测章节。
   */
  function relocateCitationMarkers(markdown, references) {
    ...
    const sanitized = sanitizeCitationMarkers(markdown, [...referenceMap.keys()]);
    const usedIds = parseCitationIds(sanitized);
    if (!usedIds.length) return sanitized;
    ...
    const anchors = lines
      .map((line, index) => ({ index, seconds: timestampSeconds(line) }))
      .filter((item) => item.seconds != null);
    if (!anchors.length) return sanitized;

    const insertions = new Map();
    for (const citationId of usedIds) {
      const reference = referenceMap.get(citationId);
      const targetSeconds = Math.max(0, Number(reference?.timestampSeconds) || 0);
      const anchor = anchors.reduce((best, candidate) => {
        if (!best) return candidate;
        const candidateDistance = Math.abs(candidate.seconds - targetSeconds);
        const bestDistance = Math.abs(best.seconds - targetSeconds);
        return candidateDistance < bestDistance ? candidate : best;
      }, null);
      if (!anchor) continue;
      if (!insertions.has(anchor.index)) insertions.set(anchor.index, []);
      insertions.get(anchor.index).push(citationId);
    }

    const output = [];
    lines.forEach((line, index) => {
      output.push(line);
      const citationIds = insertions.get(index);
      if (!citationIds?.length) return;
      output.push("", ...citationIds.map((citationId) => `{{va-cite:${citationId}}}`), "");
    });
    return output.join("\n").replace(/\n{3,}/g, "\n\n").trim();
  }
