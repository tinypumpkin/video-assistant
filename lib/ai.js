...
var BILI_AI = (() => {
  ...
  function planAnalysisChunks(
    segments,
    { maxChars = ANALYSIS_CHUNK_CHARS, overlapChars = ANALYSIS_OVERLAP_CHARS } = {},
  ) {
    ...
    if (totalChars <= ANALYSIS_SINGLE_CHARS) {
      groups.push(list);
    } else {
      let current = [];
      let chars = 0;
  ...
  /**
   * 为视频问答挑选带时间戳的字幕上下文。短视频直接使用全文；长视频同时保留
   * 首尾、均匀采样段和与问题关键词最相关的前后文，避免简单截断后只能问开头。
   */
  function buildChatTranscriptContext(
    segments,
    question,
    { maxChars = 28_000, pinnedTimestamps = [], pinnedWindowSeconds = 15 } = {},
  ) {
    ...
    if (!list.length) return "（无可用字幕）";

    const lineAt = (index) =>
      `[${formatTimestamp(list[index].start)}] ${list[index].text}`;
    const full = list.map((segment, index) => lineAt(index)).join("\n");
    const limit = Math.max(2_000, Math.floor(Number(maxChars) || 28_000));
    ...
    // 视觉手记只给模型时间点和截图；这里强制保留时间点附近的字幕，
    // 让截图在长视频被压缩时仍有可靠的语言上下文可定位。
    for (const rawTimestamp of Array.isArray(pinnedTimestamps) ? pinnedTimestamps : []) {
      const timestamp = Number(rawTimestamp);
      if (!Number.isFinite(timestamp) || timestamp < 0) continue;
      const window = Math.max(0, Number(pinnedWindowSeconds) || 15);
      let nearestIndex = 0;
      let nearestDistance = Infinity;
      for (let index = 0; index < list.length; index += 1) {
        const distance = Math.abs(list[index].start - timestamp);
        if (distance < nearestDistance) {
          nearestDistance = distance;
          nearestIndex = index;
        }
        if (distance <= window) add(index, 10_000 - Math.min(100, distance));
      }
      add(nearestIndex, 10_000);
    }

    // 首尾帮助回答「开头怎么定义」「最后结论是什么」；均匀采样兜住泛问。
    for (let i = 0; i < Math.min(10, list.length); i += 1) add(i, 60);
    for (let i = Math.max(0, list.length - 10); i < list.length; i += 1) add(i, 60);
    const sampleCount = Math.min(28, list.length);
    for (let i = 0; i < sampleCount; i += 1) {
      add(Math.round((i * (list.length - 1)) / Math.max(1, sampleCount - 1)), 20);
    }

    const query = String(question || "").toLowerCase();
    const terms = new Set(query.match(/[a-z0-9][a-z0-9_-]{2,}/g) || []);
    ...
    const scored = list
      .map((segment, index) => {
        ...
        for (const term of terms) {
          if (haystack.includes(term)) score += Math.max(2, term.length);
        }
        return { index, score };
      })
    ...
    for (const hit of scored) {
      add(hit.index, 100 + hit.score);
      add(hit.index - 1, 80 + hit.score);
      add(hit.index + 1, 80 + hit.score);
    }

    // 先按价值装预算，最后再按视频时间顺序交给模型。
    const chosen = [];
    let used = 0;
    for (const [index] of [...priorities.entries()].sort(
      (a, b) => b[1] - a[1] || a[0] - b[0],
    )) {
      const line = lineAt(index);
      if (chosen.length && used + line.length + 1 > limit) continue;
      chosen.push(index);
      used += Math.min(line.length, limit) + 1;
      if (used >= limit) break;
    }
    chosen.sort((a, b) => a - b);
    ...
    for (const index of chosen) {
      if (previous >= 0 && index > previous + 1) output.push("[…中间字幕已省略…]");
      output.push(lineAt(index));
      previous = index;
    }
    return output.join("\n").slice(0, limit);
  }

  /** 只保留最近的合法多轮消息，防止侧栏传入畸形或无限增长的历史。 */
  function normalizeChatHistory(
    history,
    { maxMessages = 12, maxChars = 12_000 } = {},
  ) {
    ...
    for (let i = cleaned.length - 1; i >= 0; i -= 1) {
      if (result.length && chars + cleaned[i].content.length > maxChars) break;
