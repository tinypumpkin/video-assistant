...
var VIDEO_YOUTUBE_API = (() => {
  ...
  async function fetchLocalTranscript(
    tracks,
    languagePreference,
    { fetchImpl = fetch, localCaptionSource } = {},
  ) {
    const available = normalizeLocalTracks(tracks);
    const track = pickLocalCaptionTrack(available, languagePreference);
    const pageSource = localCaptionSource;
    if (Array.isArray(pageSource?.transcript) && pageSource.transcript.length) {
      const language = String(pageSource.language || track?.languageCode || "");
      const result = normalizeSegments(pageSource.transcript, { language });
      if (result.transcript.length) {
        result.availableLanguages = [...new Set(available.map((item) => item.languageCode))];
        result.languageLabel = String(pageSource.languageLabel || track?.name || language);
        result.isAi = pageSource.isAi === true || track?.kind === "asr";
        return result;
      }
    }
    ...
    if (!response.ok) {
      throw makeError(`YouTube 本地字幕请求失败：HTTP ${response.status}`, "NO_SUBTITLE");
    }
    const result = typeof response.text === "function"
      ? normalizeTimedTextBody(await response.text(), track.languageCode)
      : normalizeJson3Transcript(await response.json(), track.languageCode);
    if (!result.transcript.length) throw makeError("YouTube 返回了空字幕。", "EMPTY_TRANSCRIPT");
    result.availableLanguages = [...new Set(available.map((item) => item.languageCode))];
    result.languageLabel = track.name;
    result.isAi = track.kind === "asr";
    return result;
